...
def get_connectivity_test_ips(device: Device) -> List[ipaddress.IPv4Address]:
    connectivity_test_interfaces = get_interfaces_by_condition(device, lambda interface: hasattr(interface, "connectivity_test") and interface.connectivity_test)
...
def ping_devices(source_device: Device, destination_devices: List[Device]):
    for destination_device in destination_devices:
        if source_device.name == destination_device.name:
            logger.info(f"Source device name is the same as destination device name, skipping...")
            continue
        logger.info(f"Checking connectivity to {destination_device.name} from {source_device.name}")
        destination_ips = get_connectivity_test_ips(destination_device)
        for destination_ip in destination_ips:
            if not is_ping_success_rate_higher_than_80_percent(source_device, destination_ip):
                logger.warning(f"Connectivity to {destination_device.name} from {source_device.name} is not working as expected!")
                return False
    logger.info(f"Full mesh connectivity works as expected")
    return True
        
def is_ping_success_rate_higher_than_80_percent(device: Device, destination: ipaddress.IPv4Address) -> bool:
